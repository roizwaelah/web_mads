...
export default function Header({ themeSettings }) {
  ...
  const DesktopNavLinks = ({ extraClass = "" }) => (
    <div className={`hidden lg:flex items-center h-full gap-4 font-semibold text-sm justify-end flex-1 ${extraClass}`}>
      {navItems.map((item) => {
        if (item.type === 'dropdown') {
          return (
            <div key={item.id} className="relative group h-full flex items-center">
              <button className="text-white hover:text-yellow-400 flex items-center gap-1 transition mt-1">
                ...
                <div className="bg-white border border-gray-100 rounded-xl shadow-xl overflow-hidden py-2 flex flex-col">
                  {toArray(item.children).map(child => (
                    <NavLink 
                      key={child.id} 
                      to={getPath(child)}
                      className={({ isActive }) => `text-left px-5 py-2.5 text-sm transition ${isActive ? 'bg-green-50 text-green-700 font-bold border-l-4 border-green-600' : 'text-gray-700 hover:bg-green-50 hover:text-green-700'}`}
                    >
                      {getLabel(child)}
                    </NavLink>
                  ))}
        ...
        if (item.type === 'custom') {
           return (
             <a 
               ...
               className="text-white transition flex items-center mt-1 border-b-2 h-20 border-transparent hover:text-yellow-400"
             >
               {getLabel(item)}
             </a>
           );
        ...
        return (
          <NavLink 
            ...
            className={({ isActive }) => `text-white transition flex items-center mt-1 border-b-2 h-20 ${isActive ? 'border-yellow-400 text-yellow-400 font-bold' : 'border-transparent hover:text-yellow-400'}`}
          >
            {getLabel(item)}
          </NavLink>
        );
  ...
  const LogoBlock = () => (
    <Link to="/" className="flex items-center gap-3 cursor-pointer shrink-0" onClick={() => setIsMobileMenuOpen(false)}>
      ...
      <span className="font-bold text-2xl hidden sm:block text-(--bg-site) uppercase tracking-tight">
        {settings.schoolName || 'MA Darussalam Cilongok'}
      </span>
    </Link>
  );
  ...
  const ActionBlock = ({ forceHamburger = false }) => (
    <div className="flex items-center gap-4">
      <button
        type="button"
        onClick={goToFooter}
        className="hidden sm:flex bg-(--accent,#e09d00) hover:opacity-90 text-(--bg-site) px-5 py-2 rounded-full font-bold text-sm shadow-md transition items-center gap-2 cursor-pointer"
      >
  ...
  return (
    <header className={`${isSticky ? 'fixed top-0 left-0 right-0' : 'relative'} z-50 bg-(--primary,#008e49) shadow-md text-white transition-all duration-300`}>
      ...
      {isMobileMenuOpen && (
        <div className={`${headerStyle === 'hamburger' ? '' : 'lg:hidden '}absolute left-0 w-full bg-(--primary,#008e49) border-t border-white/10 shadow-2xl pb-8 px-4 flex flex-col gap-1 overflow-y-auto max-h-[85vh] animate-in slide-in-from-top duration-300`}>
          <div className="py-4">
            {navItems.map((item) => {
              if (item.type === 'dropdown') {
                ...
                return (
                  <div key={item.id} className="mb-1">
                    <button 
                      ...
                      className="w-full flex items-center justify-between px-4 py-3 text-(--bg-site) font-bold rounded-lg bg-white/5"
                    >
                      ...
                      <div className="flex flex-col mt-1 ml-4 border-l border-white/20 pl-4 gap-1">
                        {toArray(item.children).map(child => (
                          <NavLink key={child.id} to={getPath(child)} onClick={() => setIsMobileMenuOpen(false)}
                            className={({ isActive }) => `px-4 py-2 text-sm rounded-md ${isActive ? 'bg-yellow-400 text-green-900 font-bold' : 'text-white/80'}`}>
                            {getLabel(child)}
                          </NavLink>
                        ))}
                      </div>
              ...
              if (item.type === 'custom') {
                 return (
                   <a 
                     ...
                     className="block px-4 py-3 text-base rounded-lg font-semibold transition text-white hover:bg-white/10"
                   >
                     {getLabel(item)}
                   </a>
                 );
              ...
              return (
                <NavLink 
                  ...
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={({ isActive }) => `block px-4 py-3 text-base rounded-lg font-semibold transition ${isActive ? 'bg-yellow-400 text-green-900 shadow-inner' : 'text-white hover:bg-white/10'}`}
                >
                  {getLabel(item)}
                </NavLink>
              );
          ...
          <button
            type="button"
            onClick={goToFooter}
            className="bg-(--accent,#e09d00) text-white p-4 rounded-xl font-bold flex items-center justify-center gap-3"
          >
